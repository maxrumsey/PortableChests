...
public abstract class Database {
    ...
    public String getChest(String uuid, Integer chestNo) {
        ...
        try {
            ...
            while(rs.next()) {
                if (rs.getString("player").equalsIgnoreCase(uuid.toLowerCase())) {
                    return rs.getString("chest" + chestNo);
                }
            }
        } catch (SQLException ex) {
    ...
    public void createChests(String uuid) {
        Connection conn = null;
        PreparedStatement ps = null;
        String existingChest = this.getChest(uuid, 1);
        try {
            conn = getSQLConnection();
            if (existingChest == "NOT_FOUND") {
                ps = conn.prepareStatement("INSERT INTO " + table + " (chest1, chest2, player) VALUES (?, ?, ?)");
                ps.setString(1, "");
                ps.setString(2, "");
    ...
    public void setChest(String uuid, Integer chestNo, String chestStr) {
        Connection conn = null;
        PreparedStatement ps = null;
        String existingChest = this.getChest(uuid, chestNo);
        try {
            conn = getSQLConnection();
            if (existingChest == "NOT_FOUND") {
                ps = conn.prepareStatement("INSERT INTO " + table + " (chest1, chest2, player) VALUES (?, ?, ?)");
                if (chestNo == 1) {
                    ...
                    ps.setString(2, chestStr);
                    ps.setString(1, "");
                }
                ps.setString(3, uuid);
            } else {
                ps = conn.prepareStatement("UPDATE " + table + " SET chest" + chestNo + " = ? WHERE (player = \"" + uuid + "\")");
